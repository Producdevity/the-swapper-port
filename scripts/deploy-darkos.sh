...
usage() {
  cat >&2 <<'EOF'
Usage: scripts/deploy-darkos.sh [options] <ssh-host>

Options:
  --root <path>             dArkOS ROM root. Default: /roms
  --ports-dir <path>        Full ports directory. Default: <root>/ports
...
root="${SWAPPER_DARKOS_ROOT:-/roms}"
ports_dir="${SWAPPER_DARKOS_PORTS_DIR:-}"
...
if [ -z "$autoinstall_dir" ]; then
  autoinstall_dir="$root/tools/PortMaster/autoinstall"
fi
